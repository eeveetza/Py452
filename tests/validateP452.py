...
for filename1 in filenames:
    ...
    htg    = np.double( x[0, 3] )
    hrg    = np.double( x[0, 4] )
    phi_t  = np.double( x[0, 5] )
    phi_r  = np.double( x[0, 6] )
    Gt     = np.double( x[0, 7] )
    Gr     = np.double( x[0, 8] )
    pol    = np.double( x[0, 9] )
    dct    = np.double( x[0,10] )
    dcr    = np.double( x[0,11] )
    DN     = np.double( x[0,12] ) 
    N0     = np.double( x[0,13] )
    press  = np.double( x[0,14] )
    temp   = np.double( x[0,15] )
    ha_t   = np.double( x[0,16] )
    ha_r   = np.double( x[0,17] )
    dk_t   = np.double( x[0,18] )
    dk_r   = np.double( x[0,19] )
    ...
    for i in range(0, nrows):
        Lb_ref[i]    = np.double( x[i,39] )
        Lbfsg_ref[i] = np.double( x[i,40] )
        Lb0p_ref[i]  = np.double( x[i,41] )
        Lb0b_ref[i]  = np.double( x[i,42] )
        Ldsph_ref[i] = np.double( x[i,43] )
        Ld50_ref[i]  = np.double( x[i,44] )
        Ldp_ref[i]   = np.double( x[i,45] )
        Lbs_ref[i]   = np.double( x[i,46] )
        Lba_ref[i]   = np.double( x[i,47] )
    ...
    for i in range(0,nrows):
        Lbfsg[i], Lb0p[i], Lb0b[i] = P452.pl_los(dtot, \
            ff[i], \
            pp[i], \
            b0, \
            omega, \
