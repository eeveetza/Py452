...
def bt_loss(f, p, d, h, zone, htg, hrg, phi_t, phi_r, Gt, Gr, pol, dct, dcr, DN, N0, press, temp, *args):
    """ 
    ...
    Example:
    Lb = P452.bt_loss(f, p, d, h, zone, htg, hrg, phi_t, phi_r, Gt, Gr, pol, dct, dcr, DN, N0, press, temp)
    Lb = P452.bt_loss(f, p, d, h, zone, htg, hrg, phi_t, phi_r, Gt, Gr, pol, dct, dcr, DN, N0, press, temp, ha_t, ha_r, dk_t, dk_r)
...
def  smooth_earth_heights(d, h, htg, hrg, ae, f):
    ...
    theta_rd = 1000 * np.arctan( (hts - hrs) / (1000 * dtot ) - dtot / (2*ae) )  # not defined in the recommendation
...
def great_circle_path(Phire, Phite, Phirn, Phitn, Re, dpnt):
    ...
    r = np.sind(Phitn) * np.sind(Phirn) + np.cosd(Phitn) * np.cosd(Phirn) * np.cosd(Dlon)

    # Calculate the path length as the angle subtended at the center of
    # average-radius Earth (H.2.3)

    Phid = np.acos(r)  # radians
    ...
    x1 = np.sind(Phirn)-r*np.sind(Phitn)

    # Calculate the quantity y1 (H.2.5b)

    y1 = np.cosd(Phitn)*np.cosd(Phirn)*np.sind(Dlon)
    ...
    s = np.sind(Phitn)*np.cos(Phipnt) + np.cosd(Phitn)*np.sin(Phipnt)*np.cosd(Bt2r)

    # The latitude of the intermediate point is now given by (H.3.3)

    Phipntn = np.asind(s) # degs

    # Calculate the quantity x2 (H.3.4a)

    x2 = np.cos(Phipnt)-s*np.sind(Phitn)

    # Calculate the quantity y2 (H.3.4b)

    y2 = np.cosd(Phitn)*np.sin(Phipnt)*np.sind(Bt2r)
...
def closs_corr(f, d, h, zone, htg, hrg, ha_t, ha_r, dk_t, dk_r):
    """
    closs_corr: clutter loss correction according to P.452-16
